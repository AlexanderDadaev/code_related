...
class TutorialBot(gamelib.AlgoCore):
	def on_game_start(self, config):
		...
		FILTER, ENCRYPTOR, DESTRUCTOR, PING, EMP, SCRAMBLER =  [config['unitInformation'][inx]["shorthand"] for idx in range(6)]
	...
	def build_defenses(self, location_list, firewall_unit, game_state, row=None):
		for location in location_list:
			...
			if game_state.can_spawn(firewall_unit, location):
				...
				game_state._player_resourses[0]['cores'] -= game_state.type_cost(firewall_unit)
	...
	def defense(self, game_state):
		...
		if not self.built_defenses(filters, FILTER, game_state):
			return

		row = 11
		destructors = [2, 25, 6, 21, 11, 16]
		if not self.built_defenses(destructors, DESTRUCTOR, game_state, row=row):
			return

		filters = [3, 24, 4, 23, 5, 22, 7, 20, 8, 19, 9, 18, 10, 17, 12, 15]
		if not self.built_defenses(filters, FILTER, game_state, row=row):
			return
